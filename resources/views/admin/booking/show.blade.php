...
@section('content')
    ...
    <div class="container-fluid py-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('admin.bookings.index') }}">All Bookings</a></li>
                <li class="breadcrumb-item active" aria-current="page">
                    Date Schedule: <span class="fw-bold">{{ formatDate($booking->schedule->date_time_start) }} at
                        {{ formatDate($booking->schedule->date_time_start, 'time') }} - {{ formatDate($booking->schedule->date_time_end, 'time') }}</span>
                </li>
            </ol>
        </nav>
                                    ...
                                    <ul class="nav nav-pills nav-fill flex-column flex-md-row" id="tabs-icons-text" role="tablist">
                                        <li class="nav-item">
                                            <a class="nav-link mb-sm-3 mb-md-0 active" id="tabs-icons-text-1-tab" data-toggle="tab" href="#tabs-icons-text-1" role="tab" aria-controls="tabs-icons-text-1" aria-selected="true">
                                                <i class="fas fa-cog mr-2"></i>Manage Booking
                                            </a>
                                        </li>
                                        @if ($booking->status !== 2)
                                            <li class="nav-item">
                                                <a class="nav-link mb-sm-3 mb-md-0" id="tabs-icons-text-2-tab" data-toggle="tab" href="#tabs-icons-text-2" role="tab" aria-controls="tabs-icons-text-2" aria-selected="false">
                                                    <i class="far fa-calendar mr-2"></i>Move Schedule
                                                </a>
                                            </li>
                                        @endif
                                    </ul>
                                    ...
                                    <div class="card-body">
                                        <div class="tab-content" id="myTabContent">
                                            <div class="tab-pane fade show active" id="tabs-icons-text-1" role="tabpanel" aria-labelledby="tabs-icons-text-1-tab">
                                                <form action="{{ route('admin.bookings.update', $booking) }}" method="post" id="booking_form">
                                                    @csrf @method('PUT')
                                                    ...
                                                    <div class="form-group mb-3">
                                                        ...
                                                        <select class="form-control" name="status" required>
                                                            <option value=""></option>
                                                            <option value="1" @if ($booking->status == 1) selected @endif>Approve Request</option>
                                                            <option value="2" @if ($booking->status == 2) selected @endif>Cancel Request</option>
                                                        </select>
                                                    </div>
                                            ...
                                            <div class="tab-pane fade" id="tabs-icons-text-2" role="tabpanel" aria-labelledby="tabs-icons-text-2-tab">
                                                <form action="{{ route('admin.bookings.update', $booking->id) }}" method="post" id="move_schedule">
                                                    @csrf @method('PUT')
                                                    <div class="form-group mb-3">
                                                        <select class="form-control" name="schedule_id" required>
                                                            ...
                                                            @foreach ($schedules as $schedule)
                                                                <option value="{{ $schedule->id }}">
                                                                    ...
                                                                    {{ formatDate($schedule->date_time_start, 'time') }} - {{ formatDate($schedule->date_time_end, 'time') }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    ...
                                                    <input type="hidden" name="old_schedule" value="{{ $booking->schedule_id }}">
                                                    <button class="btn btn-primary float-end" type="button"
                    ...
                    <h3>Appointment Results <i class="fas fa-clipboard-list ml-1"></i>
                        <div class="float-right">
                            <a class="btn btn-sm btn-primary me-3"
                                href="{{ route('admin.bookings.results.create', $booking) }}">Create
                                Result +
                            </a>

                            <a class="btn btn-sm btn-primary"
                                href="{{ route('admin.print.handle') }}?records=result&booking={{ $booking->id }}">Print
                                            ...
                                            <div class='dropdown'>
                                                ...
                                                <div class='dropdown-menu dropdown-menu-right dropdown-menu-arrow'>
                                                    <a class='dropdown-item'
                                                        href='{{ route('admin.bookings.results.show', [$booking, $result]) }}'>View
                                                    </a>
                                                    <a class='dropdown-item'
                                                        href='{{ route('admin.bookings.results.edit', [$booking, $result]) }}'>Edit
                                                    </a>
                                                    ...
                                                    <form
                                                        action="{{ route('admin.bookings.results.destroy', [$booking, $result]) }}"
            ...
            <div class="col-md-12">
                <div class="card card-body">
                    <h3>Prescriptions <i class="fas fa-prescription ml-1"></i>
                        <div class="float-right">
                            <a class="btn btn-sm btn-primary me-3"
                                href="{{ route('admin.bookings.prescriptions.create', $booking) }}">Create
                                Prescription +
                            </a>

                            @if ($prescriptions->isNotEmpty())
                                <a class=" btn btn-sm btn-primary me-3"
                                    href="{{ route('admin.print.handle') }}?records=prescription&booking={{ $booking->id }}">Print
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class='dropdown'>
                                                    ...
                                                    <div class='dropdown-menu dropdown-menu-right dropdown-menu-arrow'>
                                                        <a class='dropdown-item'
                                                            href='{{ route('admin.bookings.prescriptions.edit', [$booking, $prescription]) }}'>Edit
                                                        </a>
                                                        <a class='dropdown-item' href='javascript:void(0)'
                                                        ...
                                                        <form
                                                            action="{{ route('admin.bookings.prescriptions.destroy', [$booking, $prescription]) }}"
                                                            method="POST"
